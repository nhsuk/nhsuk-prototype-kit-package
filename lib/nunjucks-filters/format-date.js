...
export function formatDate(input, options) {
  ...
  try {
    ...
    const zoned = zonedDateTimeFromObject(input, resolvedOptions.timeZone)

    if (zoned) {
      date = zoned.toPlainDate()
    } else if (
      isObject(input) &&
      'year' in input &&
      'month' in input &&
    ...
    } else if (typeof input === 'string' && isIsoDateTimeWithOffset(input)) {
      date = zonedDateTimeFromOffsetString(
        input,
        resolvedOptions.timeZone
      ).toPlainDate()
    } else if (typeof input === 'string' && isIsoDateTimeWithoutOffset(input)) {
