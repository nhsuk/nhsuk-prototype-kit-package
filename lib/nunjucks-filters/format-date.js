 ...
 *
 * @param {DateInputType | object} input - Object to check for year, month and day
...
function parseDate(input, timeZone) {
  if (isIsoDateTimeWithOffset(input) || isDateInputWithOffset(input)) {
    if (typeof input === 'string') {
      return zonedDateTimeFromOffsetString(input, timeZone).toPlainDate()
    }

    return zonedDateTimeFromObject(input, timeZone).toPlainDate()
  } else if (isIsoDateTimeWithoutOffset(input) || isDateInput(input)) {
    if (typeof input === 'string') {
