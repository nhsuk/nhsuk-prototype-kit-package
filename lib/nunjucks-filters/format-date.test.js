...
describe('formatDate', () => {
  ...
  it('displays an error if the date is invalid', () => {
    ...
    assert.equal(
      formatDate({ year: 2026, month: 8, day: true }),
      'Invalid date'
    )
    assert.equal(formatDate({ year: 2026, month: 8, day: [] }), 'Invalid date')
    assert.equal(formatDate({ year: -20, month: 8, day: 1 }), 'Invalid date')
    ...
    assert.equal(formatDate({ year: 2026, month: 8, day: 7.5 }), 'Invalid date')
    // Offset without hour/minute can't be zoned, so this must throw rather
    // than silently ignore the offset and format as a plain date
    assert.equal(
      formatDate({ year: 2026, month: 8, day: 1, offset: '+00:00' }),
      'Invalid date'
    )
  })
  ...
  it('converts to local date when the ISO string contains a timezone offset (default: Europe/London)', () => {
    ...
    // As above but using an object input
    assert.equal(
      formatDate({
        year: 2026,
        month: 8,
    ...
    // UTC in winter (GMT) → same day in London (lowercase z is also valid)
  ...
  it('converts to a specific timezone using the timeZone option', () => {
    assert.equal(
      formatDate('2026-08-01T03:00Z', { timeZone: 'America/New_York' }),
      '31 July 2026'
    )
    assert.equal(
      formatDate('2026-08-01T03:00Z', { timeZone: 'Asia/Tokyo' }),
      '1 August 2026'
    )
  })
