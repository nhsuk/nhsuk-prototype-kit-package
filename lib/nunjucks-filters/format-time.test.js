...
describe('formatTime', () => {
  ...
  it('displays 12:00pm instead of midday if middayMidnight: false and includeMinutesOnTheHour: true', () => {
    assert.equal(
      formatTime('12:00', {
        middayMidnight: false,
  ...
  it('displays 12:00am instead of midnight if middayMidnight: false and includeMinutesOnTheHour: true', () => {
    assert.equal(
      formatTime('00:00', {
        middayMidnight: false,
        includeMinutesOnTheHour: true
  ...
  it('formats time objects with string values using the 24 hour clock', () => {
    assert.equal(
      formatTime({ hour: '9', minute: '5' }, { '24hour': true }),
      '09:05'
    )
  })
  ...
  it('formats ISO 8601 date-times using the 24 hour clock', () => {
    assert.equal(formatTime('2026-08-08T09:05', { '24hour': true }), '09:05')
  })

  it('formats time strings using the 24 hour clock', () => {
    assert.equal(formatTime('09:05', { '24hour': true }), '09:05')
  })

  it('formats 12 hour clock time strings using the 24 hour clock', () => {
    assert.equal(formatTime('2:15pm', { '24hour': true }), '14:15')
    assert.equal(formatTime('11:30 AM', { '24hour': true }), '11:30')
  })

  it('displays 12:00 and 00:00 rather than midday and midnight using the 24 hour clock, even if middayMidnight is true', () => {
    assert.equal(formatTime('12:00', { '24hour': true }), '12:00')
    assert.equal(
      formatTime('12:00', { '24hour': true, 'middayMidnight': true }),
      '12:00'
    )
    assert.equal(
      formatTime('00:00', { '24hour': true, 'middayMidnight': true }),
      '00:00'
    )
  })
  ...
  it('always shows minutes as 2 digits using the 24 hour clock, even if includeMinutesOnTheHour is false', () => {
    assert.equal(
      formatTime('09:00', { '24hour': true, 'includeMinutesOnTheHour': false }),
      '09:00'
    )
    assert.equal(
      formatTime('09:00', { '24hour': true, 'includeMinutesOnTheHour': true }),
      '09:00'
    )
  })

  it('displays an error if the time is invalid using the 24 hour clock', () => {
    assert.equal(formatTime('', { '24hour': true }), 'Invalid time')
    assert.equal(formatTime(false, { '24hour': true }), 'Invalid time')
    assert.equal(
      formatTime({ hour: 25, minute: 12 }, { '24hour': true }),
      'Invalid time'
    )
    assert.equal(
      formatTime({ hour: 14, minute: 61 }, { '24hour': true }),
      'Invalid time'
    )
    assert.equal(
      formatTime({ hour: '', minute: '' }, { '24hour': true }),
      'Invalid time'
    )
    assert.equal(
      formatTime({ hour: ' ', minute: ' ' }, { '24hour': true }),
    ...
    assert.equal(formatTime('25:12', { '24hour': true }), 'Invalid time')
  })
})

describe('formatTime24Hour', () => {
  it('is an alias for formatTime with the 24hour option', () => {
    assert.equal(
      formatTime24Hour('2:15pm'),
      '14:15'
    )
  })
})
