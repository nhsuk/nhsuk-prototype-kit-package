...
describe('formatTime', () => {
  ...
  it('corrects AM/PM to lowercase with no space', () => {
    assert.equal(formatTime('03:45 PM'), '3:45pm')
  })
  ...
  it('converts times with a timezone offset to local time (default: Europe/London)', () => {
    ...
    // UTC during GMT (winter) → London is UTC+0 (lowercase z designator)
    ...
    // Same as the assertion above but using object syntax
    assert.equal(
      formatTime({
        year: '2026',
        month: '12',
  ...
  it('converts to a specific timezone using the timeZone option', () => {
    // UTC during EDT (summer) → New York is UTC-4
    assert.equal(
      formatTime('2026-08-08T14:15Z', { timeZone: 'America/New_York' }),
      '10:15am'
    )
    // UTC during JST → Tokyo is UTC+9
    assert.equal(
      formatTime('2026-08-08T14:15Z', { timeZone: 'Asia/Tokyo' }),
      '11:15pm'
    )
    // UTC during CET (winter) → Paris is UTC+1
    assert.equal(
      formatTime('2026-12-08T14:15Z', { timeZone: 'Europe/Paris' }),
      '3:15pm'
    )
    // Same as above but using object syntax
    assert.equal(
      formatTime(
        {
          year: '2026',
  ...
  it('timeZone option overrides process.env.TZ', () => {
    process.env.TZ = 'America/New_York'
  ...
  it('displays an error if the time is invalid', () => {
    ...
    assert.equal(formatTime({ hour: ' ', minute: ' ' }), 'Invalid time')
    assert.equal(formatTime({ hour: null, minute: 30 }), 'Invalid time')
    assert.equal(formatTime({ hour: [], minute: 30 }), 'Invalid time')
    assert.equal(formatTime({ hour: {}, minute: 30 }), 'Invalid time')
    assert.equal(formatTime('25:12'), 'Invalid time')
    assert.equal(formatTime('14:61'), 'Invalid time')
    assert.equal(formatTime('+01:00'), 'Invalid time')
    assert.equal(formatTime({ hour: 14, minute: 12.5 }), 'Invalid time')
  })

  it('displays an error if a timezone offset is given without a date', () => {
    assert.equal(formatTime('14:15Z'), 'Invalid time')
    assert.equal(formatTime('14:15+05:30'), 'Invalid time')
    // Same as above but using object syntax
    assert.equal(
      formatTime({ hour: 14, minute: 15, offset: '+05:30' }),
      'Invalid time'
    )
  })
