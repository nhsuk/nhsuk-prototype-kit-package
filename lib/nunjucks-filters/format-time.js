...
/**
 * @param {string|TimeInputObject|unknown} input - Time as ISO string or object with hour, minute (and optionally year, month, day, offset)
 * @param {string} [zone] - IANA time zone to convert to
 * @returns {Temporal.PlainTime} the parsed time
 */
function parseTime(input, zone) {
  // 1. Object with a full date and offset: convert to the target time zone
  if (
    isObject(input) &&
    'year' in input &&
    'month' in input &&
    ...
    return sourceDateTime.withTimeZone(zone || getTimeZone()).toPlainTime()
  }

  // 2. Object with hour and minute only: a plain time with no time zone
  ...
  // 3. String input: 12 hour clock, offset date-time, or plain ISO time
 ...
 *   `useMiddayMidnight` to display 12:00 and 00:00 as midday and midnight (default: true),
 *   `zone` an IANA time zone to convert to (overrides process.env.TZ),
 *   `use24hour` to use the 24 hour clock, e.g. `09:05` or `14:15` (default: false)
 */
 ...
 * @property {string|number} [year] - The year (required with `offset`)
 * @property {string|number} [month] - The month (required with `offset`)
 * @property {string|number} [day] - The day (required with `offset`)
 * @property {string} [offset] - Timezone offset, e.g. `+00:00` (requires a full date)
 */
 ...
 * @property {string} [zone] - An IANA time zone to convert to (overrides process.env.TZ)
