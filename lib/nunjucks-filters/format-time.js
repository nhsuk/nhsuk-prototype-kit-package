...
function parseTime(input, timeZone) {
  ...
  if (
    ...
    return zonedDateTimeFromObject(input, timeZone).toPlainTime()
  }

  // 2. Object with an offset but no full date: can't be zoned, so this
  // correctly throws (invalid), mirroring the date-less offset string case
  if (isObject(input) && 'offset' in input) {
    throw new RangeError('Invalid input')
  }

  // 3. Object with hour and minute only: a plain time with no time zone
  ...
  // 4. String input: 12 hour clock, offset date-time, or plain ISO time
