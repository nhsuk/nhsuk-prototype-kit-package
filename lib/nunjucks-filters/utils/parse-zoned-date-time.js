 ...
 *
 * @param {unknown} input - Value to check for year, month, day, hour, minute and offset
 * @param {string} [timeZone] - IANA time zone to convert to (defaults to `getTimeZone()`), if the input includes a time zone offset or Z
 * @returns {Temporal.ZonedDateTime | undefined} The date-time in the target time zone, or `undefined` if the input isn't a zoned object
 */
