...
function shouldIgnoreField(fieldName) {
  return fieldName.indexOf(IGNORED_PREFIX) === 0
}

/**
 * Check if a value indicates an unchecked checkbox
 *
 * @param {unknown} value
 * @returns {boolean}
 */
function isUncheckedValue(value) {
  return value === UNCHECKED_VALUE
}
...
function storeData(input, data) {
  for (const fieldName in input) {
    ...
    if (isUncheckedValue(fieldValue)) {
