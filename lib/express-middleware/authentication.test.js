  ...
  describe('when password is set', () => {
    ...
    describe('submitting password', () => {
      it('should set cookie and redirect on correct password', async () => {
        ...
        assert.ok(
          response.headers['set-cookie'][0].includes('authentication=')
        )
      })
      ...
      it('should redirect back to password page with error on wrong password', async () => {
        ...
        assert.ok(
          response.headers.location.includes('returnURL=%2Fdashboard')
        )
      })

      it('should redirect to undefined when returnURL is not provided', async () => {
        const response = await request(app)
          .post('/prototype-admin/password')
          .type('form')
        ...
        assert.strictEqual(response.statusCode, 302)
        // Note: This test documents a bug - returnURL is undefined, so redirect goes to 'undefined'
        // Ideally, this should default to '/' when returnURL is not provided
