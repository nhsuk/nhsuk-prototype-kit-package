...
/**
 * NHS App frontend path (optional, not installed by default)
 */
export let nhsappFrontendPath

/**
 * NHS App frontend version (optional, not installed by default)
 */
export let nhsappFrontendVersion

try {
  const nhsappFrontendPkgPath = import.meta.resolve(
    'nhsapp-frontend/package.json'
  )

  nhsappFrontendPath = dirname(fileURLToPath(nhsappFrontendPkgPath))

  const nhsappFrontendPkg = JSON.parse(
    readFileSync(fileURLToPath(nhsappFrontendPkgPath), 'utf8')
  )

  nhsappFrontendVersion = `v${nhsappFrontendPkg.version}`
} catch {
  // NHS App frontend not installed in this prototype, so leave path/version undefined
}
...
if (existsSync(envPath)) {
  // The process.loadEnvFile function was experimental but added to
  // Node in 22.21.0
