...
import * as nunjucksGlobals from './nunjucks-globals/index.js'
...
export class NHSPrototypeKit {
  ...
  static async init(initOptions) {
    ...
    // Add the prototype kit Nunjucks globals
    nunjucksGlobals.addAll(nunjucks)

    // Add additional custom Nunjucks globals
    if (options?.globals) {
      utils.addNunjucksGlobals(nunjucks, options.globals)
    }
   ...
   * @returns {Promise<Partial<NHSPrototypeKit> & Pick<NHSPrototypeKit, 'app' | 'nunjucks' | 'buildOptions' | 'start'>>}
   */
  static async watch({ app, nunjucks, buildOptions }) {
    return {
      app,
      nunjucks,
      buildOptions,
      ...
      async start(port) {
        /**
         * Check Express.js app in options for preconfigured port,
         * for example when using `PORT` environment variable
         *
         * @type {number | undefined}
         */
        let proxyPort = app.get('port')
        let serverPort

        try {
          proxyPort ??= await utils.findAvailablePort(port)
          serverPort = await utils.getAvailablePort(proxyPort + 1)
        } catch (error) {
          // Port unavailable or user declines to change port
        ...
        // Prevent Nodemon restart for entry points
        if (buildOptions?.entryPoints) {
          for (const entryPoint of [buildOptions.entryPoints].flat()) {
            if (typeof entryPoint === 'string') {
              nodemonConfig.ignore.push(entryPoint)
            } else if ('in' in entryPoint) {
              nodemonConfig.ignore.push(entryPoint.in)
            }
          }
        }
 ...
 * @property {NunjucksGlobals | ((env: Environment) => NunjucksGlobals)} [globals] - Additional custom Nunjucks globals
 ...
 * @import { NunjucksGlobals } from './utils/add-nunjucks-globals.js'
 */
