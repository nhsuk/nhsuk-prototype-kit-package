 ...
 */
function getSuggestedPorts(startPort = 3000) {
  const portMin = Math.ceil(1024 / 1000) * 1000
  const portMax = Math.floor(49151 / 1000) * 1000

  // Limit start port to allowed range only
  startPort = Math.min(Math.max(startPort, portMin), portMax)

  return (
    [0, 1000, 2000, 3000, 4000, 5000, 6000]
      .map((increment) => startPort + increment)

      // Exclude ports outside the allowed range
      .filter((port) => port >= portMin && port <= portMax)
